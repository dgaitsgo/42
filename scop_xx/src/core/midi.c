...
void	copy_sysex_message(t_midi_message *m)
{
	...
	memcpy(m->sysex_message + m->sysex_length, m->packet->data, m->length_to_copy);
...
int		init_midi(t_scop *scop)
{
	...
	if (result != noErr)
		return (0);
	result = MIDIDestinationCreate(midi_client, CFSTR("Virtual port"), midi_input_callback, NULL, &end_point);
	if (result != noErr)
		return (0);
	result = MIDIInputPortCreate(midi_client, CFSTR("Input port"), midi_input_callback, scop, &input_port);
	if (result != noErr)
		return (0);
